...
class DataPreprocessing(object):
    ...
    def _samplewise_zero_center(self, batch):
        ...
        batch = tf.reshape(batch, [shape[0], np.prod(shape[1:]]))
        batch = batch - tf.reduce_mean(batch, axis = 1, keep_dims=True)
        return tf.reshape(batch, shape)
    ...
    def _samplewise_stdnorm(self, batch):
        ...
        shape = batch.get_shape().as_list()
        batch = tf.reshape(batch, [shape[0], np.prod(shape[1:]]))
        batch = batch / (tf.std(batch, axis=1, keep_dims=True))
        return tf.reshape(batch, shape)
...
class ImagePreprocessing(DataPreprocessing):
    ...
    def _crop_center(self, batch, shape):
        ...
        oshape = batch.get_shape().as_list()[1:]
